...
for i in range(5):
    inimigos.append({"pos": {"x": randint(50, 750), "y": 50}, "vel": {
                    "x": 0, "y": randint(1, 4)}, "raio": 15, "cor": (255, 0, 0)})
balas = []
ang = 45
vel = 30
maxBalas = 5
...
def desenhaBala(bala):
    pygame.draw.circle(tela, (100, 100, 0),
                       (bala["pos"]['x'], bala["pos"]['y']), 5, 0)
...
def atirar(personagem):
    if len(balas) < maxBalas:
        b = {"pos": {"x": personagem["pos"]["x"] + personagem["raio"],
                     "y": personagem["pos"]["y"]},
             "vel": {"x": int(vel * cos((pi / 180) * ang)),
                     "y": -int(vel * sin((pi / 180) * ang))}}
        balas.append(b)
...
def atualiza():
    ...
    global contIni
    if contIni % tempo == 0:
        inimigos.append({"pos": {"x": 760, "y": 250 + 50 * randint(-2, 6)}, "vel": {
